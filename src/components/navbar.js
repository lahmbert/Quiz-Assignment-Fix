...
export default function Navbar({ isOpenBars, setIsOpenBars }) {
  ...
  const [isEnLocale, setIsEnLocale] = useState(true);
  const router = useRouter();
  const pathName = usePathname();
  const [user, setUser] = useState(null);
  let locales = Cookies.get('NEXT_LOCALE') || 'en';
  const t = useTranslations('common');

  useEffect(() => {
    const cookieLocale = Cookies.get('NEXT_LOCALE') || 'en';
    setLocale(cookieLocale);
    setIsEnLocale(cookieLocale === 'en');
  }, []);
          ...
          <div className="sm:flex hidden items-center justify-around gap-4">
            ...
            <div
              className={`cursor-pointer ${
                getActiveClass(`/en/take-quiz`) ||
                getActiveClass('/ar/take-quiz')
              }`}
            >
            ...
            <div
              className={`cursor-pointer ${
                getActiveClass(`/en/list-quiz`) ||
                getActiveClass('/ar/list-quiz')
              }`}
            >
